...
export default async function AdminApplicationDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-6 py-16">
      <p className="mb-3 text-sm font-medium uppercase tracking-wide text-green-700">Application responses</p>
      <h1 className="text-4xl font-semibold">{application.full_name}</h1>
      <p className="mt-2 text-zinc-600 capitalize">{application.applicant_type} application · {statusLabels[application.status] ?? "Submitted"}</p>
      <div className="mt-10 grid gap-4 border-y border-zinc-200 py-6 md:grid-cols-3">
        ...
        {application.school && <div><p className="text-sm text-zinc-500">School</p><p className="font-medium">{application.school}</p></div>}
      </div>
      {team && <section className="mt-10 rounded-xl border border-zinc-200 p-6">
        ...
        <div className="mt-5 divide-y divide-zinc-200 border-y border-zinc-200">
          {typedTeamMembers.map((member) => <div key={member.member_id} className="flex items-center justify-between gap-4 py-4">
            ...
            <span className="text-sm capitalize text-zinc-600">{statusLabels[member.member_status] ?? "Submitted"}</span>
          </div>)}
        </div>
      </section>}
